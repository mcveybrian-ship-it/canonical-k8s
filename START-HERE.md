...
| **02** | Base OS install, **4** bare-metal hosts — **host-4 first**, it carries MAAS | [`docs/02-host-install.md`](docs/02-host-install.md) + [`airgap-media.md`](docs/airgap-media.md) | **READY** — two decisions first, §1 and §10 · **`host-1..3` were Windows machines — clear the TPM in firmware and settle Secure Boot BEFORE installing, runbook §6.3i.1. PCR 7 measures Secure Boot state, so sealing with it off and enabling it later breaks every unlock at once** |
