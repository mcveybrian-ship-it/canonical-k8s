...
| **02** | Base OS install, **4** bare-metal hosts — **host-4 first**, it carries MAAS | [`docs/02-host-install.md`](docs/02-host-install.md) + [`airgap-media.md`](docs/airgap-media.md) | **READY — and as of 2026-09-17 nothing in the repo blocks it.** `host-4` **DONE** 2026-09-01. §1a rewritten: LUKS **is** in the template (Q19/Q20 answered, proven on host-4); the GRUB password is applied in step 05, not at install, so **Q21 gates unattended reboots, not the build**. §4a settles the install path — **seed stick builds them, MAAS redeploys them**, per runbook §4 *"lifecycle, not initial build"*. §4b lists the four values that differ per host. **Remaining inputs are all physical:** boxes racked · `01-hw-inventory.sh` for the disk serials · at each firmware screen set UEFI, **clear the TPM** (ex-Windows, BitLocker may own it) and settle **Secure Boot** — runbook §6.3i.1 |
...
| **05** | Harden hosts — Pro, FIPS, STIG | [`docs/runbook.md`](docs/runbook.md) **§6.0 is the ordered procedure** (steps 13a–13d add **Evaluate-STIG**); §6.1–6.3h is the reference, §10.1 for the scanner | **IN PROGRESS** — `svc-harbor-01` **COMPLETE 2026-09-10**: 51/68 → **210 pass / 7 fail**, and the 7 are policy decisions, not defects. Next: `svc-mgmt-01`, then `svc-repo-01`, then `host-4` (§6.3 explains that order). **Five decisions outstanding** — ufw, GRUB password, SSSD ×2, and the `encrypt_partitions` write-up. See `docs/open-questions.md` |
