...
| **06a** | **PostgreSQL HA on three guest VMs** — one per physical host, Patroni with its own etcd, synchronous replication | [`docs/runbook.md`](docs/runbook.md) **§9a** | **DESIGNED 2026-09-16, not built.** Decided: application state does NOT live in the cluster. Matches Brian's Azure production pattern. The enclave already proves the argument — `svc-mgmt-01`'s archive PostgreSQL 16 is STIG-covered and patched, while Harbor's containerised **18.3 has no STIG at all** and is invisible to `dpkg`. Apps connect with a multi-host `libpq` string (`target_session_attrs=read-write`), so **no load balancer is needed**. ⬜ Blocked on one input: how many databases, and is this one app's state or a shared service — that sets the sizing |
