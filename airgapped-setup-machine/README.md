...
| **2** | ⚠️ **ONE RE-SCAN OWED: `host-4` on V1R6.** ✅ `svc-mgmt-01` (09-14), `svc-repo-01` (09-16, **NF=170 O=5 NR=9**) and `host-4` (09-16, **NF=168 O=5 NR=11**) all scanned and collected. host-4 needs **one more** run to pick up three changes made after it: the `/mnt/vmbackup` AIDE exclusion (V-270650 was timing out), the new V-270817 answer, and confirmation V-270682 stays closed | It is the last measurement in a long chain of fixes. Until it runs, four of 2026-09-16's changes are claims rather than results |
...
| **4** | The four *engineering* Not Reviewed: **V-270651** AIDE config integrity (needs the pristine `aide-common` .deb), **V-270747** data-at-rest write-up, **V-270719** PPSM, **V-270754** the ufw 443 decision — ⚠️ **this one is ALSO in Q25's signature list, and both are partly right: the test here is engineering and unrun, the acceptance of its result is the AO's. Corrected 2026-09-17** | The other NR are AO — open-questions **Q25/Q26** |
...
| ~~**9**~~ | ✅ **DONE 2026-09-16.** 51 packages from the local mirror over TLS (proven: 51 requests from 10.2.20.158 in svc-repo-01's access log), **0 pending** after, FIPS kernel 6.8.0-138-fips untouched, all four guests autostarted. **The reboot exposed Q20**: host-4 stops at a LUKS passphrase, so the whole enclave was down until somebody typed it |
| **13** | ⚠️ **THE AO PACKAGE IS NOW FIVE — send as ONE thread** | ✅ **Q20 ANSWERED 2026-09-16**: TPM 2.0 recommended for production, passphrase supported, both parameterised as `LUKS_UNLOCK`; zero of 194 V1R6 controls mention TPM so it was never a compliance trade. Remaining: **Q25** five NR controls needing a signature · **Q26** notification, 34 rules firing at a screen nobody watches at 03:00 · **Q27** Trivy DB dated 2026-09-08 · **Q28** `esm-apps` off enclave-wide · **reference clock** (a GPS/radio stratum-0 source closes both chrony rules on host-4 with no client changes) |
| **10** | **Q27 — Trivy's vulnerability DB is dated 2026-09-08 and cannot be updated from inside** | Harbor keeps reporting images clean against frozen data. Needs an AO answer on acceptable data age, which then sets the transfer cadence and `AL_TRIVY_DB_STALE`. The alert first fires **2026-10-08** |
| **11** | **Q28 — `esm-apps` is off on all four in-gap machines**, 24–67 uncovered universe packages each, plus 24–46 third-party packages with no stream at all | Needs an answer to "who patches these". If the answer is nobody, it belongs in the SSP as a named list |
| **12** | **Carry a Trivy CLI in** (`docs/00-downloads.md`, Not downloaded yet) | Nothing can scan the machines' own filesystems for vulnerabilities — Trivy exists only inside Harbor's container. Bounded by Q27 either way |
| **7** | ✅ **VM BACKUP IS BUILT AND RUNNING — 2026-09-14.** 5 TB WD easystore wiped, LUKS, ext4, mounted at `/mnt/vmbackup`, 4.6 TB usable. First full of all four VMs in progress | **Measured sizing: 386 GB total** — `svc-repo-01` 331 GB, `svc-mgmt-01` 34, `svc-harbor-01` 15, `svc-obs-01` 7.2. Cross-checks exactly against `df`. ⬜ **Owed:** `vm-backup.sh keyfile` (needs the passphrase typed), then `schedule --at 02:00`. ⚠️ **A reboot of `host-4` breaks the destination** — usb-storage is STIG-blocked, LUKS is locked, nothing mounts it. `reattach` is the one-command recovery; the timer fails loudly rather than writing to the root disk. ⚠️ **The destination is on the same machine as the VMs**, which is not really a backup — production needs storage that survives `host-4`. Runbook §10b |
| **8** | ⚠️ **110 files were lost migrating the WD drive, deliberately** | Of 83,795 files in `old_storage`, 110 returned `EACCES` from `ntfs-3g` despite normal permissions — private keys, `.ppk` files and client documents, almost certainly **Windows EFS-encrypted** and unreadable by any Linux tool. 83,685 landed on the transfer SSD and were verified before the drive was wiped. **Recoverable only from a Windows machine with the original EFS certificate, and the source no longer exists** |
...
| # | What | Notes |
|---|---|---|
| 1 | ~~Re-audit `svc-mgmt-01`~~ | ✅ **DONE 16:26 — 209 pass / 7 fail** (`usg-results-20260910.1626.xml`). The apt hook closed `file_permissions_var_log_stig`. **The 7 are rule-for-rule IDENTICAL to `svc-harbor-01`'s 7** — two machines with nothing in common converging on the same residual set, all of it policy decisions |
...
| 3 | ~~**`svc-repo-01` — §6.0 steps 1–13d**~~ | ✅ **DONE 2026-09-11. USG 212 pass / 5 fail — the LOWEST in the enclave.** V1R6 went 14 Open → 8 via one `v1r6 --apply`, then the GRUB password closed V-270675, the last HIGH that was ours. The 5 USG fails are all decisions on record: `encrypt_partitions`, `ufw_rate_limit`, `service_sssd_enabled`, `sssd_enable_user_cert`, `auditd_offload_logs`. ⬜ **Owed: re-scan V1R6 to confirm 8→7**, 3 Answer File entries (751/681/699) would take it to 4, then 14 (copy reports off) and 15 (service check). ⬜ Press `e` at GRUB on some later reboot to prove the editor is locked |
| 4 | ~~**`host-4` — full §6.0 pass**~~ | ✅ **DONE 2026-09-11. USG 210 pass / 7 fail. V1R6 8 Open → 4, and ZERO actionable findings** — the first machine to reach that. The 4 are all AO: smart-card family ×3 plus ufw on a bridging host. `encrypt_partitions` PASSES here (real LUKS). ⬜ **New AO question: does the programme supply a reference clock?** The chrony pair fails on USG because this is the time master; `time-sync.sh master --upstream` closes it with no client change. ⬜ Owed: 14 (copy reports off), 15 (service check) |
...
| E3b | **Triage `svc-mgmt-01`'s V1R6 residual** | ✅ **COMPLETE 2026-09-11 — 20 Open → 6.** **7 real fixes:** V-270645 timesyncd · V-270750 sticky bit (scanner-created) · V-270676 `audit=1` (grub.d drop-in) · **V-270675 GRUB password (HIGH)** · **V-270714 `nullok` (HIGH)** · V-270757 journal dirs `0640` (tmpfiles drop-in beat the live vendor `Z ~2750` rule — measured) · plus `audit_backlog_limit` taking `lost` 460→0. **8 answer-file entries:** 6 false positives + V-274868 (ISSO-documented exception) + V-270751 (chrony — no DoD time source exists in the gap). **EVERY finding examined.** The 6 left: CAC family ×3 (AO) · ufw (MAAS port table) · audit offload (`svc-log-01`) · V-274870 (audit rule, needs reboot). ⬜ **Reboot verification owed for V-270757** — batch with the console retrofit restart. runbook §10.1d |
...
| E5 | **Answer Files — REBUILT PORTABLE 2026-09-11. 17 entries, 0 `ResultHash` — counted 2026-09-17** with `grep -oE 'V-[0-9]{6}' answerfile.sh` piped to `sort -u`. ⚠️ This cell previously said 16 in one clause and 15 in the next, while the memory index said 17; only the last was right. **Note for anyone editing these tables: a `\|` inside inline code STILL ends the cell** — which is how this row got broken and repaired on 2026-09-17 | ⚠️ The hand-built file had `ResultHash` on 7 of 8 entries and **empty ValidationCode**, so it answered only on `svc-mgmt-01` — V-270699/703/751 came back Open on every other machine. ✅ `scripts/enclave/answerfile.sh` now generates **16 portable entries**, no ResultHash, each running DISA's CheckText verbatim at scan time and re-opening the control if the condition changes. **2026-09-14: five of them answer NOT REVIEWED controls** — V-278917 (vendor-supported release, HIGH), V-270748 (sudo group membership, HIGH), V-270816 (one week of audit storage, **measured per machine at scan time** — the first version used svc-harbor-01's 2.44 MB/day as a CONSTANT and would have returned NOT A FINDING on svc-mgmt-01, which generates ~430 MB/day and holds ~2 hours of history), V-270694 (banner acknowledgement) and V-270682 (no temporary accounts exist). Every one was executed under `pwsh` on both branches before shipping. **MEASURED on `svc-obs-01` 2026-09-14: NR 14 → 9, NF 166 → 171**, Open unchanged at 4 — exactly the five entries firing, including both HIGHs. The 9 that remain split into 4 engineering and 5 AO — open-questions Q25/Q26. ⚠️ **`ExpectedStatus` had been hardcoded to `O`** — an answer for an `NR` control with that set generates cleanly, validates cleanly and never fires. `verify` fails if a hash reappears. V-274868 deliberately unmanaged (ISSO judgement, not a technical check). Artefact stays outside the repo; the generator is in it. Runbook §10.1 |
...
| E7 | Tool distribution — **mechanism DONE 2026-09-11** | ✅ `scripts/enclave/stig-tools.sh` publishes the scanner + pwsh tarball to the mirror at `/tools/` (184 MB) and any in-gap machine runs `fetch` — checksum-verified, and it RUNS pwsh rather than assuming. The Answer File goes point-to-point over ssh, never published: the vhost has no auth and the file records security posture. Runbook §10.1. ✅ **DONE 2026-09-14:** `build-transfer-bundle.sh` now carries `tools/` — Evaluate-STIG (390 files), the PowerShell runtime (624), the tarball and its checksums, and a `stig-content/` slot for the XCCDFs. Dry run: **1,016 tool files**. `TOOLS_DIR` is a parameter in `transfer-params.env`. ✅ **And a from-scratch rebuild no longer needs an answer file to exist** — `answerfile.sh` seeds from `Template_AnswerFile.xml`, which ships inside the scanner tree and therefore travels on the same media, then strips the vendor's `V-00000` placeholder. Proven by generating into an empty path: 15 entries, no ResultHash. ⚠️ The one hand-written entry (V-274868) is NOT regenerable and must be restored separately; the script says so on every seed. **And any STIG content the bundle does not ship is now a CAC-only download** — `public.cyber.mil` redirects to SAML and the downloads index is a JS portal with no scrapable links (checked 2026-09-13). Put the XCCDFs on the manifest |
| — | 3 scanner bugs | ⚠️ **CORRECTED 2026-09-11 — they only crash on an UNHARDENED machine.** `V-270663`, `V-270729`, `V-270732` throw `InvokeMethodOnNull` on `stage-01` (all NR) but evaluate fine on `svc-mgmt-01` (NF/NF/Open). Both password-policy checks read config that does not exist until `usg fix` creates it. **Nothing to do on a hardened machine** — and the earlier claim that they were manual reviews everywhere generalised from one machine |
...
| # | What | State |
|---|---|---|
| M1 | Prometheus + Alertmanager + Grafana on `svc-obs-01` | ✅ **7 of 7 targets up.** Prometheus, Alertmanager **and Grafana** all on loopback; nginx terminates TLS on 443 and proxies to `127.0.0.1:3000`. Datasource provisioned from a file **with a pinned uid** — without one Grafana invents a random uid and every panel reads "No data" |
...
| M6 | Everything reproducible | ✅ `monitoring.sh {exporter\|libvirt\|collector\|rules\|facts\|facts-timer\|dashboards}`. Scrape config **generated** from `enclave-addresses.env`; dashboards and rules are files in the repo, not Grafana database state. Full rebuild sequence: [`docs/compliance/dashboards-and-metrics.md`](../docs/compliance/dashboards-and-metrics.md) §8 |
...
| M10 | `svc-obs-01` name resolution | ✅ **FIXED, verified 2026-09-14.** `svc-obs-01.enclave.internal` resolves to `10.2.20.164` from `host-4`, `svc-mgmt-01`, `svc-repo-01` and `svc-harbor-01` — checked on all four. Fixed with `apply-addresses.sh apply` per machine (`push` cannot work: no passwordless sudo in the enclave, by design). **The lesson generalises:** composed VMs get the `/etc/hosts` block from cloud-init, machines that already existed when a new VM appears do not, and nothing announces it |
| M8 | Dashboards | ✅ **2026-09-15 — FIVE**, provisioned from files with `allowUiUpdates: false`: `enclave-os` (21 panels), `enclave-compliance` (39), `enclave-hypervisor` (24), `enclave-collector` (30), `enclave-registry` (20). Every metric and panel documented in [`docs/compliance/dashboards-and-metrics.md`](../docs/compliance/dashboards-and-metrics.md) |
| M11 | Compliance facts as metrics | ✅ **2026-09-14.** `enclave-facts.timer` every 15 min on all five machines via node-exporter's **textfile** collector, plus the **systemd** collector narrowed to named units. Publishes STIG residual, USG results, `auditd` loss, certificate expiry, AIDE, faillock and sudo counts. **A missing source emits no samples rather than a zero** — a green zero from a scan that never ran is indistinguishable from a clean machine |
| M12 | Alert rules | ✅ **34 in 9 groups** in Prometheus (not Grafana — its DB is not versioned and dies with the VM), each carrying an `action` annotation: availability, cpu, memory, filesystems, audit trail, compliance drift, backups, registry, patch posture. Every expression evaluated against live data before shipping. ⬜ **No notification path** — V-270818/819 require email, which cannot leave an air gap (Q26) |
| M13 | Patch posture | ✅ **2026-09-15.** `pro security-status` + `apt-check` per machine, with the apt metadata date beside them — **nothing measured patch state before this**; USG and Evaluate-STIG assess configuration, not whether a package has a known CVE. Findings: `host-4` 1 security update / 51 pending, `esm-apps` **off on all four** in-gap machines (24–67 uncovered universe packages each), metadata 15 days old, 24–46 third-party packages per machine with no security stream at all |
| M14 | Registry scanning posture | ✅ **2026-09-15.** Harbor component health from its unauthenticated `/api/v2.0/health`, plus Trivy DB freshness from `metadata.json`. **Measured: DB present but built 2026-09-08**, already past its own `NextUpdate`, and unupdatable from inside — Harbor keeps reporting images clean against frozen data. **Q27**, AO + engineering |
| M15 | Vulnerability scanning of the machines | ⬜ **NOT DONE, and there is no CLI to do it with.** Trivy exists only inside Harbor's container. Either run it from that image against a bind-mounted host filesystem, or carry a binary in. Bounded by the same dated DB as Q27 either way |
...
| **FIPS + STIG on `host-1`, `host-2`, `host-3`** | ✅ **ALL THREE COMPLETE 2026-09-17, and all three at `213 pass / 3 fail`** against the enclave tailoring file — the best tally in the enclave and **the only machines where every finding is a decision rather than a defect**: `auditd_offload_logs` (three AO answers), `sssd_enable_user_cert` (the CAC decision), and `service_sssd_enabled` (**we caused it deliberately** — a daemon that cannot start in an air gap; owes a written deviation). Baseline was `54/63` on each. **`check_ufw_active` and `grub2_uefi_password` PASS here** — they were never completed on the earlier five, which is why those carry six findings and these carry three; see `HANDOFF.md` §3. **`host-3` was driven end to end by `05-harden-host.sh` and reached 213/3 first time with nothing to re-apply**, because it patched at step 3b BEFORE hardening. `host-1` and `host-2` were hardened first and a 185-package upgrade reverted three controls — 213/3 → 211/5 — recovered by re-applying `fixups` and `v1r6` (runbook §10d). ⬜ Still owed on all three: `stig-tools.sh collect`, `audit-volume.sh install`, and the step-15 functional test, which is blocked behind the `datavg`/`pool` sizing question |
...
| **bin/** | **24 M** | 2 | **Added 2026-09-08** — `regctl` + `regsync`, statically linked, with `SHA256SUMS`. Nothing inside the gap could push an OCI artifact before this |
