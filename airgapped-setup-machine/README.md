...
| 3 | **`svc-repo-01` — 13a–13d DONE 2026-09-11** | ✅ USG **211 pass / 6 fail**. Evaluate-STIG V1R6 **14 Open → 8** via one `v1r6 --apply` + reboot, verified (exit 0). Of the 8: **1 actionable (V-270675 GRUB password, in flight)**, 3 Answer File (751/681/699), 4 AO decisions. Answer-filing the three takes it to 5. ⬜ **Owed: `--unrestricted` in `/etc/grub.d/10_linux` is ABSENT here** — must go in BEFORE `superusers`, or GRUB demands the password to boot and the mirror never comes back unattended. Then 14 (copy reports off) and 15 (service check) |
