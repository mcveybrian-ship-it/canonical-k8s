...
| 4 | **`host-4` — full §6.0 pass, LAST** | Rebooting it takes every VM. The USB toggle is what makes it survivable. **Step 8c is not optional here** — `/var/lib/libvirt/images` is ~1.9 TB and `usg fix` builds the AIDE database inline (§6.3h) **GRUB password is the enclave-wide one from the controlled document** - same as harbor/mgmt/repo, not a new secret (§6.3i) |
