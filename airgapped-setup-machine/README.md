...
| **FIPS + STIG on `svc-harbor-01`** | ✅ **DONE 2026-09-08** — FIPS kernel from the enclave mirror, `usg fix stig-v1r1` applied, rebooted, re-audited. **51 pass / 68 fail → 205 pass / 11 fail.** The 11 are the findings register — runbook §6.3. ⚠️ **`usg fix` LOCKED THE ADMIN ACCOUNT OUT OF SUDO** (no password + STIG removes NOPASSWD). Fixed at source in `03-compose-vm.sh`; `vm-rescue.sh` gets you back in. §6.3a. **Of the 11, only 2 were build defects** — both chrony, both answered in §6.3b: `maxpoll 16` ✅ **FIXED and verified on the wire**, and the approved-server list **TAILORED** via `stig-tailor.sh` (⏳ audit not yet run). The other 9 are decisions, a compensating control, or an accept |
