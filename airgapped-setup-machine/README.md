...
| **FIPS + STIG on `svc-harbor-01`** | ✅ **HARDENED 2026-09-08/10.** `usg fix stig-v1r1`, reboot, re-audit, chrony remediation + tailoring, then the `/var/log` fixups. **51/68 → 205/11 → 207/9 → ⏳ tally of `usg-results-20260910.0135.xml` not yet read.** ⚠️ **`usg fix` LOCKED THE ADMIN ACCOUNT OUT OF SUDO** — fixed at source in `03-compose-vm.sh`, `vm-rescue.sh` is the way back in. §6.3a. **All 4 build defects now fixed:** `maxpoll 16` ✅ on the wire; approved-server list (**UBTU-24-600160**) ✅ tailored; `wtmp/btmp/lastlog` ✅ `0640` via `/etc/tmpfiles.d/var.conf`, **PROVEN across a reboot 2026-09-10 01:34** (the entire point of the tmpfiles route); apt logs ✅ `create 0640` in logrotate. **DECISION MADE: `rsyslog` installed** — real group, legitimate pass; does NOT close `auditd_offload_logs`. §6.3c. Remaining 7 rules = the five decisions in `docs/open-questions.md` |
