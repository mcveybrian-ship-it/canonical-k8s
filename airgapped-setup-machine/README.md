...
| **FIPS on `svc-mgmt-01`** | ✅ **DONE 2026-09-10** — `fips_enabled=1`, kernel `6.8.0-138-fips`, all from the enclave mirror. **MAAS survived intact:** all 10 services active, `dhcpd` on `:67` and `rackd` TFTP on `:69`, so the PXE path is unbroken; PostgreSQL still accepts connections (`scram-sha-256`, checked before FIPS precisely because `md5` is not FIPS-approved); chrony still `^* host-4`. Pre-checks that mattered: admin password was **locked** (composed 2026-09-03, before the composer required one) — set first; MAAS's 4 sudoers files backed up to `/var/backups/stig-tailor/` because `usg fix` strips `NOPASSWD` and MAAS needs it for `maas-dhcpd` and `lshw`. Next: `pro enable usg`, baseline audit, `preflight`. runbook §6.0 |
