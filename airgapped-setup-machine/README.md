...
| **7** | **First real VM backup run** — `vm-backup.sh status` on `host-4`, then a full to the 4 TB external drive | ⚠️ **USB storage is STIG-blocked on `host-4`** — a plugged-in drive will not appear until `stig-tailor.sh usb --allow`, and the window gets closed again after. The destination is a parameter (`--dest`, `VM_BACKUP_DEST`, or `BACKUP_DEST` in `vm-specs.env`) so the bench drive and a customer mount are the same procedure. ⬜ The open problem is the DESTINATION: every VM is on `host-4` and so is the only free space, which is not a backup. Runbook §10b |
