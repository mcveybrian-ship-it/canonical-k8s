...
| 4 | ~~**`host-4` — full §6.0 pass**~~ | ✅ **USG DONE 2026-09-11: 210 pass / 7 fail** (`usg-results-20260911.1820.xml`). FIPS, AIDE exclusion held (362 GB out of scope), fixups+v1r6+GRUB password in ONE reboot. `encrypt_partitions` PASSES here (real LUKS); the chrony pair fails because it is the time master — **new AO question: does the programme supply a reference clock?** ⬜ **Owed: 13b V1R6 scan (running), 13c triage, 14 copy reports off, 15 service check.** ⬜ Also owed: re-audit harbor + mgmt, whose numbers predate their GRUB password |
