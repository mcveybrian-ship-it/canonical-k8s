...
| **6** | Push to `origin` — **60+ commits ahead** | — |
...
| M8 | Dashboards | ✅ **2026-09-14.** Two provisioned from files with `allowUiUpdates: false` — `enclave-os` (21 panels: fleet, OS, filesystems, guests from the hypervisor) and `enclave-compliance` (30 panels: STIG residual, audit trail, integrity and access, expiry). Every metric documented in [`docs/compliance/dashboards-and-metrics.md`](../docs/compliance/dashboards-and-metrics.md) |
...
| M12 | Alert rules | ✅ 8 in Prometheus (not Grafana — its DB is not versioned and dies with the VM), each carrying an `action` annotation. ⬜ **No compliance alerts yet**, and no notification path: V-270818/819 require email, which cannot leave an air gap (Q26) |
