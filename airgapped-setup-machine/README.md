...
| 3 | **`svc-repo-01` — full §6.0 pass, IN FLIGHT** | Pro attached, FIPS on (`6.8.0-138-fips`), baseline **54 pass / 65 fail**, preflight clean, `usg fix` complete. **Remaining: step 8c's AIDE exclusion then `aide init`** — `aide --init` was killed mid-`fix` walking the 321 GB mirror, so the database is absent. Then service checks, reboot, `usg audit`, 13a–13d |
