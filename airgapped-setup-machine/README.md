...
| E3b | **Triage the 20 + 14 on `svc-mgmt-01`** | ✅ **DONE 2026-09-11 01:55 UTC — 20 Open → 8.** 162 NF / 14 NR / 10 NA. **5 real fixes** (V-270645 timesyncd · V-270750 sticky bit, scanner-created · V-270676 `audit=1` via grub.d drop-in · **V-270675 GRUB password, HIGH** · **V-270714 `nullok`, HIGH** — needed edits to BOTH `common-auth` and `pam-configs/unix`) **+ 7 answer-file entries** (6 false positives where the scanner is stricter than its own CheckText, plus V-274868 as a documented ISSO exception). **The 8 left are all known decisions:** CAC family ×3 (AO) · ufw (MAAS port table) · auditd offload (`svc-log-01`) · V-274870 (audit rule, needs a reboot) · V-270751 + V-270757 unexamined. runbook §10.1d |
...
| E5 | **Answer Files for our deviations** | ✅ **BUILT + PROVEN 2026-09-11** — 7 entries at `/srv/bundle-staging/tools/answerfiles/Ubuntu24_AnswerFile.xml`, passed with `--AFPath`. **Kept OUTSIDE the repo deliberately** — it carries security posture (locked root, 60 NOPASSWD grants) and `origin` is public; also keeps us clear of the licence's no-modify term. **7 live entries, 6 pinning `ResultHash`**, so an answer expires the moment the finding changes rather than masking it forever. ⬜ Still to add: V-270751 (chrony) once its FindingDetails are read, and ⚠️ **it is NOT covered by `private-sync`** — needs a backup route |
