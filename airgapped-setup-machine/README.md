...
| E5 | **Answer Files for our deviations** | ✅ **BUILT + PROVEN 2026-09-11** — 7 entries at `/srv/bundle-staging/tools/answerfiles/Ubuntu24_AnswerFile.xml`, passed with `--AFPath`. **Kept OUTSIDE the repo deliberately** — it carries security posture (locked root, 60 NOPASSWD grants) and `origin` is public; also keeps us clear of the licence's no-modify term. **8 live entries, 7 pinning `ResultHash`**, so an answer expires the moment the finding changes rather than masking it forever. ✅ V-270751 added and verified `NF`. ⚠️ ⚠️ **it is NOT covered by `private-sync`** — needs a backup route |
