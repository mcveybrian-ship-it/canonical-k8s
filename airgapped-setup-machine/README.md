...
| 3 | **`svc-repo-01` — RESUME HERE 2026-09-12** | USG **211 pass / 6 fail** (final). Evaluate-STIG V1R6 **14 Open + 14 NR**, then `v1r6 --apply` closed 645/750/699/714/757. **ONE THING BLOCKING:** `update-grub` exits non-zero for an unknown reason (its output was swallowed; the script now prints it). `/boot/grub/grub.cfg` is INTACT and bootable — nothing is at risk, and **do not reboot until grub-mkconfig succeeds.** Also: `GRUB_CMDLINE_LINUX=" audit=1"` was present in `/etc/default/grub` before the apply and is absent after — read the backup in `/var/backups/stig-tailor/` rather than reasoning about which `sed` did it. First command tomorrow: `sudo grub-mkconfig -o /dev/null 2>&1 \| tail -20`. Then V-274870 + V-270757 need the reboot, then `v1r6 --verify`, then 13d re-scan, 14 copy reports off, 15 service check |
| 4 | **`host-4` — full §6.0 pass, LAST** | Rebooting it takes every VM. The USB toggle is what makes it survivable. **Step 8c is not optional here** — `/var/lib/libvirt/images` is ~1.9 TB and `usg fix` builds the AIDE database inline (§6.3h) |
