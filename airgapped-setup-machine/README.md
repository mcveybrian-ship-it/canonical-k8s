...
| 3 | **`svc-repo-01` — §6.0 steps 1–13 DONE 2026-09-11** | ✅ **211 pass / 6 fail** (`usg-results-20260911.0343.xml`) — the residual set **minus `check_ufw_active`**, which closed legitimately because this is the first machine with a real ufw table and no Docker bypass. AIDE exclusion seeded at step 8c, database 18 MB / 74,436 entries with the 321 GB mirror out of scope. Tooling now fetched from the mirror (E7). **Remaining: 13b scan, 13c triage, 13d re-scan, 14 copy reports off, 15 service check** |
