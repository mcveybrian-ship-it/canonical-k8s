...
| **FIPS + STIG on `svc-harbor-01`** | ✅ **HARDENED 2026-09-08/09.** `usg fix stig-v1r1`, reboot, re-audit, then chrony remediation + tailoring. **51 pass / 68 fail → 205/11 → 207 pass / 9 fail.** ⚠️ **`usg fix` LOCKED THE ADMIN ACCOUNT OUT OF SUDO** (no password + STIG removes NOPASSWD) — fixed at source in `03-compose-vm.sh`, `vm-rescue.sh` is the way back in. §6.3a. **4 of the 11 were build defects, 2 closed:** `maxpoll 16` ✅ fixed and verified on the wire, approved-server list (**UBTU-24-600160**) ✅ tailored via `stig-tailor.sh`. ⏳ **STILL OPEN — the `/var/log` pair** (`file_permissions_var_log_stig`, `file_groupowner_var_log`): fixable, probably Harbor, §6.3c. The other 7 rules are the five decisions in `docs/open-questions.md` |
