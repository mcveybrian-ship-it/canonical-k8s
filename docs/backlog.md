...
| **6a.20** | **🔴 Crunchy-vs-community Postgres is an AO call, and it changes the workload** | The only PostgreSQL 16 STIG is **Crunchy Data's** — 334 mentions of "Crunchy", RHEL paths, pgaudit-dependent, **zero** Patroni awareness. The plan is community Postgres + Patroni on Ubuntu. Whether the AO accepts a *tailored Crunchy STIG* as "the product STIG", or rules the product uncovered and demands the full Database SRG, is **111 tailored rules + 34 delta vs 142 generic**. Get it in writing early | ⬜ **AO decision, get it early** |
