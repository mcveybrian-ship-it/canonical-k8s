...
| **3.13** | **MAAS leftovers in `stig-tailor.sh` — one weakens the baseline** | Found 2026-09-19 writing svc-mgmt-01's ufw table. MAAS was removed 2026-09-18 but three things kept its shape: **line 90 deselects `sudo_require_authentication` on svc-mgmt-01 because MAAS shipped its own NOPASSWD sudoers files** — with MAAS gone that deviation is a hole with no reason; line ~2907 excludes `/var/lib/maas/boot-resources` from a check; line ~3189 describes isc-dhcp/tftp. **Fix:** confirm the MAAS sudoers files are gone on svc-mgmt-01, drop the deviation, regenerate its tailoring file, re-audit 🔄 **2026-09-19: deviation and AIDE exclusion removed from the script** (root check: no NOPASSWD anywhere, no maas account). **Cleanup on svc-mgmt-01 found more than MAAS: 61 unowned files.** 54 under `/var/lib/maas` incl. MAAS's **cluster private keys**, 4 under `/var/lib/bind/maas` (MAAS's old zones), and **BIND's own `managed-keys` files owned by the DELETED bind account** — the MAAS autoremove took bind9, the reinstall created `bind` as 106:109 instead of 110:114, and named had logged `managed-keys.bind.jnl: open: permission denied` since 2026-09-18 unnoticed. Removed / re-grouped; named restarted clean, resolves, NXDOMAIN correct, **orphans 0**. ⚠️ A guard grepping all of `/etc/bind` stopped on a `.bak` file — the right check is `named-checkconf -p` (the effective config). **Remaining:** regenerate tailoring, audit, scan | 🔄 cleanup done · rescan next |
