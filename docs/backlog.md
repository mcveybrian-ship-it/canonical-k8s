...
| **3.12** | **nginx `server_name` carries a literal `${ENCLAVE_DOMAIN:-enclave.internal}`** | Found 2026-09-19 while writing the PPSM services list. `enable-tls.sh:225` escaped the `$`, so every generated config says `server_name svc-X svc-X.${ENCLAVE_DOMAIN:-enclave.internal} _;` — measured on svc-mgmt-01, svc-repo-01, svc-obs-01. Harmless in practice (one TLS server block per machine answers every name) but an assessor reads it as untested config. **Script fixed 2026-09-19**; the live files change the next time `enable-tls.sh` runs on each machine | 🔄 script fixed · 3 live configs to refresh |
