...
| **4** | 🔴 **PPSM CAL — `cyber.mil/ppsm/cal/`. ~4 files on the page; WHICH ONE is not yet known** | Added 2026-09-18 (it had been claimed as added before it was). DISA's FAQ says only that the CAL lists data services by assurance colour (Green/Yellow/Red) with a "Temporary" section for services still in VA. The page is CAC-only and search does not list its files, so Brian reads the file names off the page. **Take the CAL itself, the whole list rather than a filtered view**, plus any exceptions list. Put them in `docs/compliance/stigs/` (gitignored) | 6a.22 — the CLSA's "on the CAL / not on the CAL" column |
