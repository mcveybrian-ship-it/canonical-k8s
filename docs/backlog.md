...
| | Item | Notes | Status |
|---|---|---|---|
| **3.1** | **Write host-4's and `svc-mgmt-01`'s ufw rule tables**, then enable | ⏸️ **Deferred by decision 2026-09-18.** NOT a one-liner: both default policies are DROP and host-4's table has two rows with **no `22/tcp`** — enabling as-is drops SSH on a machine with no BMC and cuts bridged traffic for every guest. `svc-mgmt-01`'s MAAS port list is still unconfirmed. 🆕 **2026-09-19: the svc-mgmt-01 half is now easy** — MAAS is gone, and the PPSM generator measured what it actually listens on: 22, 53 tcp+udp (named), 80/443 (nginx), 123/udp (chrony), 9100, plus 8484 (contracts server, meant to be loopback-only — prove with nmap). host-4 is still the hard half | ⏸️ deferred · svc-mgmt-01 now unblocked |
