...
| **6b.1c** | 🔴 **`airgapped-contracts.yaml` still says `enclave.local`** — generated 2026-08-31 before the `.internal` decision (runbook §564). **systemd-resolved routes multi-label `.local` to MulticastDNS**, so it never reaches a unicast server. It works via `/etc/hosts` today and **fails the moment anything relies on DNS** — which is exactly what 6b.1b introduces. **Fix before pointing resolvers, not after** | ⬜ **blocks 6b.1b** |
