...
| **Privacy (PTA / PIA)** | ⬜ **A PIA is required — settled 2026-09-22.** `DATA_TYPES` records PII of staff and operators, **PII of members of the public**, incidental health information and call-record metadata, so the screening outcome is not in doubt. Backlog **D-7**. ✅ The health data is incidental and this is not a health care component, so HIPAA/DoDM 6025.18 do not apply. ⚠️ A Privacy Act **SORN exists and covers this system** — a **separate artifact from the PIA**; its identifier is owed so both this and `ssp.md` can cite it (**Q-SORN**) |
