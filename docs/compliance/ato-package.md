...
| **Privacy (PTA / PIA)** | ⬜ **A PIA is required — settled 2026-09-22.** `DATA_TYPES` records PII of staff and operators, **PII of members of the public**, incidental health information and call-record metadata, so the screening outcome is not in doubt. Backlog **D-7**. ✅ The health data is incidental and this is not a health care component, so HIPAA/DoDM 6025.18 do not apply. 🔴 Whether a Privacy Act **SORN** exists is **unknown** — it is a **separate artifact from the PIA** and normally programme-owned. ⚠️ Do not infer one from a login banner: the product's page labelled *SORN* is the DoD consent banner, which is **AC-8** (**Q-SORN**) |
