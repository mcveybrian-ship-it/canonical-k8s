...
|---|---|---|---|---|---|---|---|
| ENG-01 | `ato-package.md` §1, `nist-800-53-plan.md` CP | **No contingency test of any kind has ever been performed.** No host-failure rehearsal, no guest restore, no database failover exercise. CP-4 therefore has no evidence of any kind | CP-4 | **High** | Run the host-failure rehearsal (runbook §10), restore one guest from the volume, and time both. Record what happened, including failures. This is the only item in this register that cannot be written instead of performed | «OWNER» | «TARGET_DATE» |
...
| ID | Source | Weakness | Control | Risk | Mitigation / milestone | Owner | Target |
|---|---|---|---|---|---|---|---|
| ENG-10 | HANDOFF §3a matrix | **nginx on `svc-repo-01` has no product STIG.** Per the locked 2026-09-13 decision the enclave assesses against the governing **Web Server SRG** rather than recording an absence. The SRG has not been obtained or applied | CA-2, CM-6 | Moderate | Obtain the Web Server SRG, load it through `StigContent/Manual/`, assess, write up | «OWNER» | «TARGET_DATE» |
...
| ENG-28 | `airgapped-setup-machine/README.md` §0a item 2a | **MAAS invokes `machine-resources` roughly 1.7 times per second on `svc-mgmt-01`** — 231,159 invocations in 38 hours, each one a sudo session and an audit record. It produced 5 GB/day of syslog and is what makes V-270816 fail there. All three MAAS services are `active` with zero restarts, so nothing is crash-looping and **the cause is not known** | AU-4, SI-4 | Moderate | Log rotation now bounds the damage; it does not fix it. Root-cause the invocation rate | «OWNER» | «TARGET_DATE» |
...
CP-4 has no evidence of any kind (ENG-01, ENG-05). It is the only item in this register that
