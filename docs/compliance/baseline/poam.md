...
| CUST-15 | `ssp-inputs.md` §2.1b, `backlog.md` 3.15 | **Account lockout has no break-glass path.** `deny=3` with `unlock_time=0` (correct per the STIG), `passwd_tries=1` so one typo is one strike, `pam_faillock` in the account stack so a locked account is refused **SSH keys too**, one administrator account, and no BMC on host-1/2/3: three typos in 15 minutes make a host recoverable only from its physical console. Reached 2 of 3 strikes in normal work on 2026-09-21. **No control is being relaxed** | AC-2, AC-7, AU-12, IA-5, MA-4 | **High** | Decide and implement: a second named admin account; a documented emergency admin account, console-only, credentials under dual custody, audited, rotated after use; the GRUB + LUKS console recovery path written down with its credentials in the same custody; and a BMC in the production BOM (CUST-14). Record the AO decision | «OWNER» | «TARGET_DATE» |
