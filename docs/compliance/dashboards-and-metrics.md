...
> SSP"* with nothing collecting them.
...
This document is the reference for the **five** Grafana dashboards in the enclave and for
every metric behind them.

| Dashboard | uid | Panels | Section |
|---|---|---|---|
| Enclave OS — host and guests | `enclave-os` | 21 | §3 |
| Enclave compliance | `enclave-compliance` | 39 | §4 |
| Enclave hypervisor — host-4 and VM backups | `enclave-hypervisor` | 24 | §4a |
| Enclave collector — Prometheus, Alertmanager, Grafana | `enclave-collector` | 30 | §4b |
| Enclave registry — Harbor and Trivy | `enclave-registry` | 20 | §4c | It covers what each number means, where it comes from, how often it
...
**Thirty-four rules in nine groups** in `/etc/prometheus/rules/`, generated by `monitoring.sh rules`. Every one carries
...
**Order between `collector` and `exporter` no longer matters.** It used to: `collector` set
node-exporter's `ARGS` to the listen address alone and silently undid `exporter`. Both now call
the same `ne_args()` builder, so either order leaves the same flags. Re-running `collector` on a
machine is safe again.
...
A rebuild is not finished because the six commands returned zero. Each of them verifies itself
against the running process, but the whole stack has a state worth checking deliberately:

```
# on svc-obs-01
./scripts/enclave/monitoring.sh status

curl -s http://127.0.0.1:9090/api/v1/targets?state=any \
  | python3 -c 'import json,sys; print(len(json.load(sys.stdin)["data"]["activeTargets"]), "targets")'

curl -s http://127.0.0.1:9090/api/v1/rules \
  | python3 -c 'import json,sys; d=json.load(sys.stdin)["data"]["groups"]; print(sum(len(g["rules"]) for g in d), "rules /", len(d), "groups")'
```

| Check | Expected |
|---|---|
| scrape targets | **9** — 5 node, 1 libvirt, prometheus, alertmanager, grafana |
| alert rules | **34 in 9 groups** |
| dashboards in Grafana's sqlite | **5** |
| `node_scrape_collector_success{collector=~"textfile\|systemd"}` | **1** on all five machines |
| `enclave_facts_source_ok` | present for every source a machine has; **absent is not failure** — it means that source does not exist there |
| `time() - enclave_facts_generated_seconds` | under 900 on all five |
| `node_textfile_scrape_error` | **0** on all five |

⚠️ **Check the textfile directory is on the running command line, not just that the collector
reports success.** `node_scrape_collector_success{collector="textfile"} 1` is true with no
directory configured — it succeeds at reading nothing. `monitoring.sh exporter` prints
`textfile directory is on the running command line` when it is genuinely set.
