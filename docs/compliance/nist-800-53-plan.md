...
|---|---|---|
| **AC** Access Control | ✅⚠️ | Local accounts only; `usg fix` removed blanket `NOPASSWD`; per-command sudo exceptions scoped and justified (`svc-mgmt-01` for MAAS); DoD banner before all SSH output; ufw via `stig-tailor.sh ufw`. ⚠️ **SSSD decisions still open** — two of the five hardening decisions |
