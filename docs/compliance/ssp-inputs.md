...
⚠️ **Corrected 2026-09-24 — until that date the mechanism above did not work, and the SSP should not
claim otherwise for the period before it.** Alertmanager on `svc-obs-01` was running the
package's example configuration (receivers delivering to `example.org`), and no dashboard listed
which alerts were firing. Measured, fixed and proven the same day (backlog B-09a):

| | |
|---|---|
| **Who** | The **system administrator on duty** — a role, named per site on the roster (acting AO, 2026-09-24) |
| **When** | At the **start of every duty day**. The audit-space alert fires at 75%, early enough to cover a day's gap |
| **Where** | Grafana's **home dashboard** (*Enclave OS*): the *Alerts firing* count and the *What is firing* table directly beneath it |
| **Alertmanager** | One receiver, no integrations — nothing pretends to send mail. Generated by `monitoring.sh alerting`, checked with `amtool check-config`, and verified against the **running** process |
| **Proof** | `monitoring.sh alert-test on` raises `AlertPathTest` through scrape → rule → Alertmanager; it was **seen on the home dashboard by a person** and cleared with `alert-test off`, 2026-09-24. Repeat after any change to the monitoring stack |

**The control depends on a person looking**, and that is its honest limit: an alert raised
after the day's check waits until the next one.
