...
V-270817	UBTU-24-900950	DISA's own check text says this control is not applicable to an interconnected system; every enclave machine ships its audit records over the network to the collector weekly (backlog N-2), and the answer VERIFIES that offload - timer enabled, root-owned runtime copy, last run within 8 days, last result success - rather than asserting it. The scanner reports O on most machines and NR on host-4 (a man-db script in cron.weekly it cannot judge), so the answer fires on both	O,NR
