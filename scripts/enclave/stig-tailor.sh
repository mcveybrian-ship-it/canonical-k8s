...
host-1	9100/tcp	limit	__SVC_OBS_01__	node-exporter, source-restricted to the collector. Not listening yet - monitoring has not been extended to the bare-metal hosts - but the rule is written now so enabling the exporter is not also a firewall change. LIMIT not allow (2026-09-19): the only permitted source scrapes every 15 s, ~2 connections per 30 s, well under ufw's 6-per-30 s threshold - and ufw_rate_limit fails on ANY listening port left at allow
