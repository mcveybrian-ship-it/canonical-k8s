...
cmd_verify() {
  ...
  # MAAS WAS REMOVED FROM THE BOUNDARY 2026-09-18, so :5248 is only checked where MAAS is
  # actually installed. This used to fail unconditionally, which would break every rebuild
  # from now on - the check outlived the thing it was checking.
  #
  # Kept rather than deleted because the two-nginx problem it guards is real: installing MAAS
  # stops and disables the system nginx, taking the contracts server offline for every client
  # with no error anywhere (found by chance hours later, 2026-09-08). If MAAS ever returns,
  # this check must return with it.
  if dpkg-query -W -f='${Status}' maas-rack-controller 2>/dev/null | grep -q "^install ok installed"; then
    if ss -ltn 2>/dev/null | grep -q ":5248"; then
      ok "MAAS http boot still listening on :5248"
    else
      warn "nothing on :5248 - MAAS is installed but cannot serve boot images"; fail=1
    fi
  else
    say "MAAS is not installed - :5248 not expected (removed from the boundary 2026-09-18)"
  fi

  # AND THE THING THAT REPLACED IT. MAAS was the only DNS server in the enclave; bind9 now
  # serves enclave.internal authoritatively from zones generated by apply-addresses.sh.
  # runbook 9a.4.
  if systemctl is-active --quiet named 2>/dev/null; then
    local dnsprobe
    dnsprobe="$(dig +short +time=2 @127.0.0.1 "svc-repo-01.${ENCLAVE_DOMAIN:-enclave.internal}" 2>/dev/null)"
    if [ -n "$dnsprobe" ]; then
      ok "enclave DNS answering (svc-repo-01 -> $dnsprobe)"
    else
      warn "named is running but did not answer for svc-repo-01 - run
       sudo ./scripts/enclave/apply-addresses.sh zone-install"; fail=1
    fi
  else
    warn "named is NOT running - this machine serves enclave DNS. Run
       sudo ./scripts/enclave/apply-addresses.sh zone-install"; fail=1
  fi
